...
class EmployeeProjectsController extends GetxController {
  ...
  final RxList<Map<String, dynamic>> filteredTasks = <Map<String, dynamic>>[].obs;
...
void filterTasks() {
  ...
  for (var task in tasks) {
    ...
    print("🔍   Created by: ${task['created_by']?['_id']} (${task['created_by']?['username']})");
    print("🔍   Assigned to: ${task['assigned_to']?['_id']} (${task['assigned_to']?['username']})");
  ...
  if (selectedTabIndex.value == 0) {
    ...
    filteredTasks.value = tasks.where((task) {
      ...
      if (createdBy != null && createdBy['_id'] != null) {
        ...
        print("🔍 Task '${task['task_name']}': Created by '$createdById' == '$currentUserIdTrimmed' = $match");
        return match;
      }
      return false;
    }).toList();
    ...
    print("🔍 Filtering for ASSIGNED tasks (assignee OR collaborator) - excluding completed tasks...");
    filteredTasks.value = tasks.where((task) {
      ...
      if (taskStatus == 'completed') {
        print("🔍 Task '${task['task_name']}': Status is completed - excluding from Assigned tab");
        return false;
      }
      ...
      // Task matches if user is EITHER assignee OR collaborator (or both) AND not completed
      final match = isAssignee || isCollaborator;
  ...
  if (filteredTasks.isNotEmpty) {
    ...
    if (selectedTabIndex.value == 1) {
      ...
      print("🔍   3. Check if user ID matches exactly (including case sensitivity)");
      print("🔍   4. Check if user ID is stored correctly in GetStorage");
      print("🔍   5. Verify API response structure for assigned_to and collaborators");
    }
  }
}
    ...
    try {
      ...
      switch (e.type) {
        ...
        case DioExceptionType.sendTimeout:
          errorMessage.value = "Connection timeout. Please check your internet.";
          break;
  ...
  Map<String, dynamic>? getLatestWorkDetail(List<dynamic>? workDetails) {
    ...
    try {
      final sorted = List<Map<String, dynamic>>.from(workDetails)
        ..sort((a, b) {
